...
class Filter : public Layer {
	//Filter Layer --> seperate by channels --> seperate by features 

	Filter* f_next;
	Filter* f_prev;
	void filter_link(Filter* f) {
		f_next = f; 
		f->f_prev = this;
	}
...
public:
	...
	Filter(unsigned input_length, unsigned input_width, unsigned numb_filters, unsigned filter_length, unsigned stride);
	~Filter();
